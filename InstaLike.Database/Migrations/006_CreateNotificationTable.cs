...
namespace InstaLike.Database.Migrations
{
    ...
    public class CreateNotificationTable : AutoReversingMigration
    {
        public override void Up()
        {
            ...
            Create.Column("SenderID")
                .OnTable("Notification").InSchema("dbo")
                .AsInt32()
                .NotNullable();
            Create.Column("RecipientID")
                .OnTable("Notification").InSchema("dbo")
                .AsInt32()
                .NotNullable();
            Create.Column("Message")
                .OnTable("Notification").InSchema("dbo")
                .AsAnsiString(200)
                .NotNullable().WithDefaultValue(string.Empty);
            Create.Column("ReadByRecipient")
                .OnTable("Notification").InSchema("dbo")
                .AsBoolean()
                .NotNullable().WithDefaultValue(false);
            Create.Column("NotificationDate")
                .OnTable("Notification").InSchema("dbo")
                .AsDateTimeOffset()
                .NotNullable().WithDefault(SystemMethods.CurrentDateTimeOffset);

            Create.PrimaryKey("PK_Notification")
                .OnTable("Notification").WithSchema("dbo")
                .Column("ID").Clustered();
