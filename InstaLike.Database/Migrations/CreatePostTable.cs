...
namespace Instalike.Database.Migrations
{
    ...
    public class CreatePostTable : AutoReversingMigration
    {
        public override void Up()
        {
            ...
            Create.Column("UserID").OnTable("Post").InSchema("dbo").AsInt32().NotNullable();
            // Picture varbinary(MAX) FILESTREAM
            Create.Column("Text").OnTable("Post").InSchema("dbo").AsString(500).NotNullable().WithDefaultValue(string.Empty);
            Create.Column("PostDate").OnTable("Post").InSchema("dbo").AsDateTimeOffset().NotNullable().WithDefault(SystemMethods.CurrentDateTimeOffset);
            // [PostGuid] [uniqueidentifier] ROWGUIDCOL  NOT NULL

            Create.PrimaryKey("PK_Post").OnTable("Post").WithSchema("dbo").Column("ID").Clustered();
